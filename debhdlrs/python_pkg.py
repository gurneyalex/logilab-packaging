...
class PythonMixIn(object):
    ...
    def __init__(self, pkginfo, replace_func, empty_func):
        ...
        if not 'python' in self.pkginfo.depends:
            self.pkginfo.depends.insert(0, '${python:Depends}')
    ...
    def _rule_python_install(self, pkginfo, package, python='python'):
        if pkginfo.subpackage_of:
            pkg_dir = '%s/%s' % (pkginfo.subpackage_of, pkginfo.modname)
        else:
            pkg_dir = pkginfo.modname
        result = """
       %(python)s setup.py -q install --no-compile --prefix=debian/%(pkgname)s/usr/ --install-headers=debian/%(pkgname)s/usr/include/ --install-scripts=debian/%(pkgname)s/usr/bin
       # remove test directory (installed in the doc directory)
       rm -rf debian/%(pkgname)s/usr/lib/python*/site-packages/%(pkgdir)s/%(testdir)s
       """ % {'python'  : python,
              'pkgname' : package,
              'testdir' : pkginfo.test_directory,
              'pkgdir'  : pkg_dir,
              }
        return result
    
    def _control_python(self, stream, short_desc, long_desc):
        pkginfo = self.pkginfo
        patterns = {
        ...
        patterns['BUILD_DEPENDENCIES'] = ', python-all'
        patterns['DEPENDENCIES'] = self._control_python_default_deps(pkginfo)
        patterns['PYVERSIONS'] = ', '.join(pkginfo.pyversions)
...
class PythonDepStandaloneHandler(PythonMixIn, DebianizerHandler):
    ...
    def rules(self, stream):
        ...
        for pyver in pkginfo.pyversions:
            ...
            patterns['INSTALL_PYTHON'] += self._rule_python_install(pkginfo, package, python)
        # data
...
class PythonIndepStandaloneHandler(PythonMixIn, DebianizerHandler):
    ...
    def rules(self, stream):
        ...
        patterns = self.base_rules()
        patterns['INSTALL_PYTHON'] += self._rule_python_install(pkginfo, debian_name)
        # data
        if pkginfo.data_files:
            patterns['INSTALL_PYTHON'] += '''
	python setup.py -q install_data --install-dir=debian/%s/usr/
